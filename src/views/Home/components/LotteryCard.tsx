...
import { Heading, Card, CardBody, Button, useModal } from '@candyswap-libs/uikit'
...
import CandyWinnings from './CandyWinnings'
...
const LotteryCard = () => {
    ...
    if (!allowance.toNumber()) {
      return (
        <Button width="100%" disabled={requestedApproval} onClick={handleApprove}>
          {TranslateString(494, 'Approve CANDY')}
        </Button>
      )
    }
    return (
  ...
  const [onPresentBuy] = useModal(<BuyModal max={candyBalance} tokenName="CANDY" />)
  ...
  return (
    <StyledLotteryCard>
      <CardBody>
        ...
        <CardImage src="/images/ticket.svg" alt="candy logo" width={64} height={64} />
        <Block>
          <Label>{TranslateString(552, 'CANDY to Collect')}:</Label>
          <CandyWinnings />
        </Block>
        <Block>
