...
import { Button, InjectedModalProps, Skeleton, Text } from '@candyswap-libs/uikit'
...
import { useCandyRabbits, useProfile as useProfileContract } from 'hooks/useContract'
import { getCandyProfileAddress, getCandyRabbitsAddress } from 'utils/addressHelpers'
...
const ChangeProfilePicPage: React.FC<ChangeProfilePicPageProps> = ({ onDismiss }) => {
  ...
  const candyRabbitsContract = useCandyRabbits()
  ...
  const {
    ...
    onApprove: () => {
      return candyRabbitsContract.methods.approve(getCandyProfileAddress(), tokenId).send({ from: account })
    },
    onConfirm: () => {
      if (!profile.isActive) {
        return profileContract.methods.reactivateProfile(getCandyRabbitsAddress(), tokenId).send({ from: account })
      }

      return profileContract.methods.updateProfile(getCandyRabbitsAddress(), tokenId).send({ from: account })
    },
    onSuccess: async () => {
      // Re-fetch profile
    ...
    <>
      ...
      {!isLoading && walletNfts.length === 0 && (
        <>
          ...
          <Text as="p" color="textSubtle" mb="24px">
            {TranslateString(999, 'Make sure you have a Candy Collectible in your wallet and try again!')}
          </Text>
        </>
      )}
