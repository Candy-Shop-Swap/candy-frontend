...
const minimumCandyToRegister = new BigNumber(REGISTER_COST).multipliedBy(new BigNumber(10).pow(18))
...
const UserName: React.FC = () => {
  ...
  const { teamId, tokenId, userName, actions, minimumCandyRequired, allowance } = useProfileCreation()
  ...
  const hasMinimumCandyRequired = useHasCandyBalance(minimumCandyToRegister)
  const [onPresentConfirmProfileCreation] = useModal(
    <ConfirmProfileCreationModal
      ...
      account={account}
      teamId={teamId}
      minimumCandyRequired={minimumCandyRequired}
      allowance={allowance}
    />,
    false,
  )
    ...
    <>
      ...
      {!hasMinimumCandyRequired && (
        <Text color="failure" mt="16px">
          {TranslateString(1098, `A minimum of ${REGISTER_COST} CANDY is required`, { num: REGISTER_COST })}
        </Text>
      )}
    </>
  )
