...
const BattleCta: React.FC<CompetitionProps> = ({
  ...
  const { hasRegistered, hasUserClaimed, userCandyRewards, userPointReward, canClaimNFT } = userTradingInformation

  const userCanClaimPrizes = !hasUserClaimed && (userCandyRewards !== '0' || userPointReward !== '0' || canClaimNFT)
  ...
  const handleCtaClick = () => {
    ...
    if (hasRegistered && isCompetitionLive) {
      window.location.href = 'https://exchange.candyswap.online/#/swap'
    }
