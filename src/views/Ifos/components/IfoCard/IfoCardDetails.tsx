...
const IfoCardDetails: React.FC<IfoCardDetailsProps> = ({ ifo, publicIfoData }) => {
  ...
  const { description, candyToBurn, projectSiteUrl, launchDate, launchTime, saleAmount, raiseAmount } = ifo
      ...
      {isOpen && (
        <>
          ...
          <Box mb="24px">
            ...
            <Item>
              <Display>{TranslateString(586, 'CANDY to burn (USD)')}</Display>
              <Text>{candyToBurn}</Text>
            </Item>
            <Item>
